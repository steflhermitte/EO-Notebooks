...
In this R notebook we will learn how to:
...
Moreover, in order to download the images, you need to install the `rgee` package. For the complete `rgee` installation instruction, I refer to the dedicated [installation instruction](https://r-spatial.github.io/rgee/#installation), but below you find a short version:

```{=plain}
install.packages("rgee")
# Additionally, rgee depends on the Python packages: numpy and ee. To install them, users can follow any of these two methods:

# Method 1: Use ee_install (Highly recommended for users with no experience with Python environments) 

rgee::ee_install() # If you don't have a python version with ee yet

# Method 2: Use ee_install_set_pyenv (Recommended for users with experience with Python environments)
...
# or for Mac users:
#rgee::ee_install_set_pyenv(
#  py_path = "/Users/u0036095/mambaforge/envs/gee/bin/python",
#  py_env = NULL
#)
```
Subsequently we can load `rgee:`

```{r install_rgee}
library(rgee)
ee_check() # to check if it is properly installed
ee_Initialize()
```

Once `rgee` is successfully installed, we still need to install 2 additional packages directly from the corresponding github pages using the `remotes` package that has the option to directly a package from a github page with the `install_github` function:

```{r}
# Install package to visualize histograms as ridge lines
...
```{r}
# Change working directory with setwd
# setwd(location/of/your/unzipped/labZipfile/)
setwd('/Users/slhermitte/Library/CloudStorage/OneDrive-KULeuven/teaching/2022-23_S2_I0D73a_RSVS/Classification/Lab')

# Control the working directory by printing it
getwd()
```

## Step 1: Pre-process remote sensing data on GEE

During the past few years, GEE has become very popular in the geospatial community and it has empowered numerous environmental applications at local, regional, and global scales. GEE provides both JavaScript and Python APIs for making computational requests to the Earth Engine servers.

Within GEE, data range from images/image collections to geometries/features/feature collections. When processing data it is important to understand the difference between images and image collections.

![](Rimages/GEE_imageVsCollection.png)

### Images in GEE

Images are individual rasters composed of one or more bands and each band has its own name, data type, scale, mask and projection. Each image has metadata stored as a set of properties. Images can be loaded by pasting an Earth Engine asset ID into the ee.Image() constructor. You can find image IDs in the data catalog. For example, to load the USGS SRTM terrain model within GEE we can:

```{r load-image}
# Define Earth Engine data set
image = ee$Image('USGS/SRTMGL1_003')
ee_print(image)
```

Subsequently, we can visualize the data within GEE:

```{r vis-image}
# Make a visualization setting with min, max and palette values
colorizedVis <- list(
  min=0.0,
  max=3000.0,
  palette=c('blue','green','brown','red'
  )
)
# visualize in GEE
Map$addLayer(image,colorizedVis,'SRTM')
```

```{r}
dataset = ee$ImageCollection('ESA/WorldCover/v200')$first()

visualization = list(bands=c('Map'))
...
### ImageCollections in GEE

An ImageCollection is a stack or sequence of images. For example, it can be the entire [Sentinel-2 surface reflection collection](https://notebooks.githubusercontent.com/view/%27https://developers.google.com/earth-engine/datasets/catalog/COPERNICUS_S2_SR%27) which contains all Sentinel-2 surface reflectance images over the Earth.
...
S2 = ee$ImageCollection('COPERNICUS/S2_SR')$
        filterBounds(ee$Geometry$Point(4.75, 50.75))$
        filterDate('2019-01-01', '2019-12-31')$
        filterMetadata('CLOUDY_PIXEL_PERCENTAGE','less_than',20)
ee_print(S2)
```

The standard way to get an image out of a collection is to reduce the collection. For example, we can sort based on a metadata field and subsequently get the first image. Notice in the following example that the image appears very dark as the min/max values are 0/10000 (corresponding to reflectance values of 0/1; see scale parameter in [data catalog](https://developers.google.com/earth-engine/datasets/catalog/COPERNICUS_S2_SR#bands)), but changing the max-value to a lower value will make it clearer.

```{r vis-collection-first}
...
Map$centerObject(S2_im,zoom=10)
```

Or apply some basic reducers to get the mean, median etc.

```{r vis-collection-median}
...
# Add image
Map$addLayer(S2_im,S2_vis,'S2 mean') + Map$addLayer(S2_im2,S2_vis,'S2 median')
```

If you toggle the layers by clicking ![](Rimages/GEE_layerButton.png) , you will notice how the reducers have an effect on the end result. For example, it is clear that the mean reducer is still heavily impacted by the clouds that bias the pixel values high.
...
```{r define-geometry}
# Point based on lon/lat which is default
point = ee$Geometry$Point(coords=c(1.5, 1.5))
# Region based on rectangle in lat/lon projections (i.e., EPSG:4326)
region.leuven <- ee$Geometry$Rectangle(
  coords = c(4.7, 50.7, 4.8, 50.8),
  proj = "EPSG:4326",
  geodesic = FALSE
)
ee_print(region.leuven)
```

These geometries can subsequently be re-projected to for example Lambert 72 projection which corresponds to [EPSG:31370](https://epsg.io/31370)

```{r transform-geometry}
region.leuven.bd72 = region.leuven$transform(ee$Projection('EPSG:31370'),0.01)
ee_print(region.leuven.bd72)
```

## Step 2: Download data from GEE

Everything we have done and visualized up to now completely happened in GEE: i.e. we did the processing (e.g., filtering, reducing) on GEE and just visualized the output with R.

However, in some situations it might also be handy to export data from GEE to your local computer by using the `ee_as_raster` function. To export, we always need to define:

-   `image`: ee\$Image to be converted into a raster object. This image also defines the coordinate reference system (`crs`) of the output

-   `region`: a region of interest (bounding box). CRS needs to be the same that the argument image. Otherwise, it will be forced. If not specified, image bounds are taken.

-   `via`: to indicate that the data are exported through Google drive. This means you need to connect your Google drive to GEE.

-   `scale`: the scale (in m) in which the data are exported

```{r im2raster}
# first only select the bands that start with a 'B' to remove the quality bands
S2_exp = S2_im2$select('B.*') 
# export as raster
s2_r <- ee_as_raster(S2_exp, 
                      region=region.leuven,
                      via = "drive",
                      scale = 10)
```

So if you want to export the data in a different projection system, you first need to re-project it:

```{r reproject_image}
S2_im2_rep = S2_im2$reproject(crs='EPSG:31370')
ee_print(S2_im2_rep)
```

## Step 3: Read in downloaded data

When running `ee_as_raster`, the data are piped through your Google drive directly in a raster stack object in memory. Afterwards, it is always good practice to save your data set to your disk in order to load it faster in later stages.

```{r write-raster}
writeRaster(s2_r,'RData/s2_r.tif',options=c('TFW=YES'))
```

This data set can subsequently be reloaded into memory

```{r load-raster}
s2_rl = brick('RData/s2_r.tif')
s2_rl
```
...
map_1 = tm_shape((s2_rl/5500*255)) +        # Set the raster layer as the main layer and scale its colors according to the values in the layer divided by 5500 and multiplied by 255
         tm_rgb(r = 4, g = 3, b = 2) +      # Use a custom RGB color scheme with red = 4, green = 3, and blue = 2
         tm_scale_bar(position = c("left", "bottom"))  # Add a scale bar to the map in the bottom-left corner

# Create another tmap object (map_2) with a raster layer (s2_rl[[4]]) overlaid 
map_2 = tm_shape(s2_rl[[4]]) +              # Set the raster layer as the main layer
        tm_raster(alpha = 0.7,              # Set the opacity of the raster layer to 0.7
...
OK, now you know how to download, read in and visualize data. In the next notebook, we will see how to further explore this data and finally classify this data into information

## Step 5: Explore the data

When using the raster::click function we can explore some of the pixel values. Therefore, however, you have to make sure the markdown is plotted in the console and not inline in the notebook. This implies clicking on the preferences wheel above and setting the Chunk Output in console:
...
ggplot()+
     ...
     geom_line(data = Pixel.melt, mapping = aes(x=wl, y=Reflectance, color=idf), lwd=1)+
     ...
     geom_point(data = Pixel.melt, mapping = aes(x=wl, y=Reflectance, color=idf), pch=20,size=5)+
...
dat_all <- as.data.table(rasterToPoints(s2_rl))
...
dat_mlt = melt(dat_all, id.vars = "id", value.name ="Reflectance")

# Create a ridgeline plot of the 'Reflectance' values for each 'id' in the 'dat_mlt' data frame
# using the 'ridgeline()' function from the 'ggridges' package.
# The 'Reflectance' values are plotted on the y-axis, and the 'id' values are plotted on the x-axis.
# The resulting plot is displayed in the output.
ridgeline(dat_mlt$Reflectance,dat_mlt$variable)
```
...
Now that we have learnt to download data from GEE (Steps 1+3) and do a prilimary analysis on it, we can actually start getting data for classification in a supervised classification exercise. Therefore, we are going to try to classify a Sentinel-2 image in different landcover classes. We are going to use the Sentinel-2 image we already downloaded as unknown input features `X` in order to classify each pixel in a class `Y`. Since we are going to do a supervised classification, we are going to need known labels to training data. In this exercise, we are going to use a earlier landcover [Worldcover](https://developers.google.com/earth-engine/datasets/catalog/ESA_WorldCover_v200) classification done by ESA as reference data.

```{r}
# Load the woldCover collection from GEE, select the Map band and convert it to an image with toBands 
lc = ee$ImageCollection("ESA/WorldCover/v200")$select('Map')$toBands()  
# Print image info
ee_print(lc)
```

Subsequentlty we can visualize these layers in GEE.

```{r}
# Create visualisation setting
colorizedVis <- list(
  min=0.0,
  max=100.0,
  palette=c(
    '006400', 'ffbb22', 'ffff4c', 'f096ff', 'fa0000', 'b4b4b4', 'f0f0f0',
    '0064c8', '0096a0', '00cf75', '00cf75','fae6a0'
  )
)

# Visualize interactively in GEE
Map$addLayer(lc, colorizedVis, 'Worldcover') + Map$addLayer(S2_im2,list(bands=c("B4","B3","B2"),max=1000,min=0),"S2") + Map$addLayer(region.leuven)
```

Now we can download this Worldcover image to select our training data from it.

```{r}
lc_r <- ee_as_raster(lc, 
                      region=region.leuven,
                      via = "drive",
                      scale = 10)
...
```{r}
# Write it to raster on your computer
writeRaster(lc_r,'./Rdata/lc_r.tif',options=c('TFW=YES'))
```

```{r}
# Read raster from computer
lc_r = raster('Rdata/lc_r.tif')
```
...
poly_dt <- as.data.table(rasterToPoints(lc_r))
...
setnames(poly_dt, old = "lc_r", new = "id_cls")
...
So this resulting table `dt` gives us for every sample the spectral reflectrance and corresponding class value. Since some pixels might still contain `NA` values that might mess up our classifiers, we need to remove the rows that contain `NA`'s.
...
We can now calcalte the mean reflectance per class
...
idx_train <- createDataPartition(dt$class, # variable to stratify by
...
folds <- createFolds(1:nrow(dt_train), k = n_folds) # create n_folds folds of the data
```
...
Subsequently, we can set the seed for each iteration of the folds in a `seeds` variable that will store the random number seeds for each resampling iteration. This ensures that each resampling iteration has a unique random seed, which can be useful when running the process in parallel.
...
-   Cross-validation: The caret package makes it easy to perform cross-validation, a key step in model evaluation and hyperparameter tuning, with support for different types of cross-validation such as k-fold, repeated k-fold, and leave-one-out cross-validation.

-   Hyperparameter tuning: The caret package provides tools for tuning hyperparameters of machine learning models, which can greatly improve model performance. The train() function in caret can automatically search through a grid of hyperparameter values and select the best-performing combination.

-   Parallelization: The caret package supports parallel computation, which can speed up model training and hyperparameter tuning on multi-core systems.
...
-   `search`: specifies the search method to be used for hyperparameter tuning. Here, grid indicates that a grid search will be used.
...
## Step8 : Random Forest classifier
...
cl <- makeCluster(3/4 * detectCores()) # detectCores() finds the number of CPU cores
registerDoParallel(cl) # register the cluster with foreach package

# create a control object for train() function to use
...
model_rf <- caret::train(class ~ . , # class ~ .: specifies the formula for the model, where class is the target variable and . indicates that all other variables in the dt_train dataset should be used as predictors.
                         ...
                         allowParallel = TRUE, # enable parallel computation during cross-validation
...
# stop and remove the parallel cluster
stopCluster(cl)
remove(cl)

# unregister the doParallel cluster so that we can use sequential operations if needed
registerDoSEQ()
...
The plot shows that the final/optimal model stored in `model_rf,` corresponds to `mtry` = 3 with the highest accuracy = 0.876.
...
-   `as.factor(dt_test$class)`: the true class labels from the `dt_test` dataset, converted to a factor variable.

```{r}
# Calculate the prediction data for the test set
dat_x = predict(model_rf, newdata = dt_test)
# Get the reference test date
dat_y = as.factor(dt_test$class)
# Calculate confusion matrix
cm_rf <- confusionMatrix(data = dat_x, dat_y)
cm_rf
```

When calculating the confusion matrix, you compare the different classes. When sampling sparse classes it could be the case that some of your classes are not represented within the prediction or test set. If this happens, the confusion matrix cannot be calculated. One solution to that is to hardcode the potential classes.

```{r}
# Set class names
dat_levels = c("X10","X30","X40","X50","X60","X80")
# Calculate the prediction data for the test set
dat_x = predict(model_rf, newdata = dt_test, levels= dat_levels)
# Get the reference test date
dat_y = as.factor(dt_test$class)
levels(dat_y)=dat_levels
# Calculate confusion matrix
cm_rf <- confusionMatrix(data = dat_x, dat_y)
cm_rf
```
...
caret::varImp(model_rf)$importance %>%
  ...
  plot_ly(x = colnames(.), y = rownames(.), z = ., type = "heatmap", width = 350, height = 300)
...
OK, so we have trained one RF model, but off course we can also train some completely different models. On the [caret help page]() you can find a complete list of model types that can be used in caret and which relevant characteristics are set there.
...
# Create a parallel processing cluster and register it with the doParallel package
...
# Stop and remove parallel processing cluster, and register sequential processing backend
...
cm_svm <- confusionMatrix(data = predict(model_svm, newdata = dt_test), as.factor(dt_test$class))
cm_svm
```

Here you notice that the SVM models performs less than the SVM model.
...
# create a parallel backend for caret using 3/4 of the CPU cores
cl <- makeCluster(3/4 * detectCores())
registerDoParallel(cl)
...
# stop the parallel backend and unregister it
stopCluster(cl); remove(cl)
registerDoSEQ()
...
```{r}
# The confusion matrix using the test dataset
cm_nnet <- confusionMatrix(data = predict(model_nnet, newdata = dt_test), as.factor(dt_test$class))
cm_nnet
```

Here you notice that the NN model performs slighty less than the RF model, but better than the SVM model.
...
```{r}
sync(viewRGB(s2_rl, r = 4, g = 3, b = 2)+mapView(lc_r),
     mapView(predict_rf), 
     mapView(predict_svm),
...
Compare the different models and discuss their performance in terms of overall accuracy and how well they spatially capture the landcover patterns.
...
In the final step we are also going to implement a unmixing example. This step is optional as installing the `RStoolbox` package needs to done from the github page and can be tricky

```{r}
# Install package of RS tools that are useful
remotes::install_github("bleutner/RStoolbox")
library(RStoolbox)
```

If succesfully installed, we can start unmixing
...
em = ct[4:15]
rownames(em) = c('forest','soil','grass','urban')

# Do the unmixing
probs <- mesma(s2_rl, em, method = "NNLS",verbose=TRUE)
```
...
```{r}
sync(viewRGB(s2_rl, r = 4, g = 3, b = 2)+mapView(lc_r),
     mapView(probs$forest)+mapView(probs$soil)+mapView(probs$grass),
     mapView(probs$RMSE))
```
...
-   Visualize both the satellite image and landcover map
-   Optimize three different landcover classifiers over your home location and assess their accuracy
...
-   Perform an unmixing over your home area and compare that with the landcover classes.
