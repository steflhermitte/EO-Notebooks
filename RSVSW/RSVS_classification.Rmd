...
```{r}
cm_rf <- caret::confusionMatrix(data = predict(model_rf, newdata = dt_test),as.factor(dt_test$class))
cm_rf
```
...
OK, so we have trained one RF model, but of course we can also train some completely different models. On the [caret help page]() you can find a complete list of model types that can be used in caret and which relevant characteristics are set there.
